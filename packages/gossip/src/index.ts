...
export const publicKeyFromPrivateKey = (privateKey: string) => {
  return Buffer.from(
    ec
      .keyFromPrivate(new Uint8Array(Buffer.from(privateKey)))
      .getPublic()
      .encode("array", false)
  ).toString("hex");
};
...
const verify = (
  ...
  const verified = key.verify(hash, signature);
  return verified;
};

const getBlake2Hash = (a: Uint8Array, length: number) =>
  blake2b(a, new Uint8Array(), length);

export const signZoneTransaction = (
  data: any,
  privateKey: string,
  format: "uint8array" | "hex"
) => {
...
export const checkZoneTransaction = (
  ...
  if (ignoreDate === true) {
    console.warn(
      "You should not ignore date, make sure you are in a test environment"
    );
  }
  if (ignoreDate === false) {
    const {
    ...
    if (typeof date === "number") {
      ...
      if (
        ...
        new Date(date).toISOString() <
          new Date(new Date().getTime() - 1000 * 60 * 10).toISOString()
      ) {
        throw new Error("Invalid date");
      }
    } else {
  ...
  const payload = new Uint8Array(
    Buffer.from(JSON.stringify(zoneTransactionWithSignature.data))
  );
  const blake2Hash64 = getBlake2Hash(payload, 64);

  if (verify(blake2Hash64, publicKey, zoneTransactionWithSignature.signature)) {
    return true;
  }
