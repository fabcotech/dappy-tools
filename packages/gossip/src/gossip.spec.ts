...
describe('gossip.ts simple typologies', function(){
  it('should gossip 2 out of 3', async function() {
    this.timeout(4000);
    ...
    expect(results).to.eql([true, true, 'undefined:undefined@undefined comm/tls']);
  });

  it('should gossip 100% after network interruption (1)', async function() {
    this.timeout(16000);
  ...
  it('should gossip 100% after network interruption (2)', async function() {
    this.timeout(16000);
    let t = new Date().getTime();
    const results = await gossip(
      [1, 2, 3] as unknown as DappyNetworkMember[],
...
describe('gossip.ts complex typologies', function(){
  ...
  it('should gossip 4 out of 4', async function() {
    ...
    const gossipOfMember3 = async () => {
      if (!member3HasReceivedMessages) {
        throw new Error('Member 3 cannot gossip if he has not received message')
      }
      const results = await gossip(
